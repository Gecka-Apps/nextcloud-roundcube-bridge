...
help:
	...
	@echo ""
	@awk '/^#@@[^@]/{desc=$$0; sub(/^#@@ /, "", desc); getline; if(/^[a-z]/) { target=$$0; sub(/:.*/, "", target); printf "  %-15s %s\n", target, desc }}' $(MAKEFILE_LIST)
	@echo ""
.PHONY: help
...
#@private
npm-init: node_modules
.PHONY: npm-init

#@private
npm-build: stamp.npm-build
stamp.npm-build: $(WEBPACK_DEPS) node_modules
	$(NPM) run build
	touch $@
.PHONY: npm-build

#@private
npm-dev: stamp.npm-dev
stamp.npm-dev: $(WEBPACK_DEPS) node_modules
	$(NPM) run dev
	touch $@
.PHONY: npm-dev
...
#@private
webpack-clean:
	rm -f stamp.npm-build stamp.npm-dev
	rm -rf js/
.PHONY: webpack-clean
